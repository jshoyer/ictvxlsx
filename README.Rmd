...
```{r}
library(taxize)
cablcv_id <- get_ids("Cabbage leaf curl virus", db = "ncbi")
```
...
I am not eager to try to use the internal site API
(if it still exists at all).

I do not anticipate
using any of the other databases
that `taxize` supports,
beyond possibly checking how consistent and up-to-date they are.
```{r}
begomo_ids_multi <- get_ids("Begomovirus")
```
