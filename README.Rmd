
My goal here
is to make it a little bit easier
to use tables
from the ICTV
via a dedicated R package.

```{r}
library(readxl)
library(dplyr)

msl2023 <-
  read_excel("ICTV_Master_Species_List_2023_MSL39.v1.xlsx",
             col_types = "text",
             sheet = 2, .name_repair = "universal")

vmr2023 <-
  read_excel("VMR_MSL39_v1.xlsx",
             col_types = "text",
             sheet = 1, .name_repair = "universal")

vmr_e <- filter(vmr2023,
                    Exemplar.or.additional.isolate == "E") |>
  select(-Exemplar.or.additional.isolate)

vmr_e_slim <- transmute(vmr_e,
                            Family, Genus, Species,
                            name = Virus.name.s.,
                            abbrev = Virus.name.abbreviation.s.,
                            isolate = Virus.isolate.designation,
                            genbank = Virus.GENBANK.accession,
                            refseq = Virus.REFSEQ.accession)
## Genome.coverage, Genome.composition, Host.source

vmr_e_slim2 <- select(vmr_e_slim,
                          -Family, -Genus)

msl2023_e <- left_join(msl2023, vmr_e_slim2,
                       by = "Species")

## Top down: ######################################################################
monodna <- filter(msl2023_e, Realm == "Monodnaviria") |>
    select(-Realm, -Subrealm)

as.data.frame(with(monodna, table(abbrev))) |>
    filter(Freq > 1) |>
    left_join(vmr_e_slim) |>
    select(abbrev, name, Species, Genus)

## Bottom up: ######################################################################
filter(vmr_e_slim2, abbrev == "AYVV") ## not quite, because

## many abbreviations have an extra identifier tacked on
filter(vmr_e_slim,
       Genus == "Begomovirus",
       ##Family == "Geminiviridae",
       grepl("/", abbrev)) |>
    select(name, abbrev, isolate)
```
